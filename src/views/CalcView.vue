...
const onFormSubmit = ({ valid }) => {
  ...
  total.value =
    7.04163367 +
    12.33223018 * Number(parameters.value) -
    0.25993187 * Number(musr.value) +
    0.35981478 * Number(mathlvl5.value) -
...
<template>
  ...
  <div>
    ...
    <Form
      ...
      :validateOnBlur="true"
      @submit="onFormSubmit"
      class="flex flex-col gap-4 w-full sm:w-2/3 md:w-1/2 lg:w-[400px]"
    >
      ...
      <Message v-if="$form.generation?.invalid" severity="error" size="small" variant="simple">{{
        $form.generation.error.message
      }}</Message>

      <Button type="submit" severity="secondary" label="Submit" class="sm:w-56" />
    </Form>
  </div>
  <p class="mt-4 text-lg font-semibold">Total CO₂ generated (kg): {{ total }}</p>
</template>
